...
The hardcoded IDs below are correct for this repo as of 2026-04-04 14:58:32 UTC. If they ever change, or if you're working in a fork, re-run these queries to get fresh values:

```bash
# Repository node ID
gh api graphql -f query='{ repository(owner: "shinyworks", name: "chains") { id } }'

# Available issue type IDs
gh api graphql -f query='{ repository(owner: "shinyworks", name: "chains") { issueTypes(first: 20) { nodes { id name description } } } }'
```
...
| Type | ID | Use for |
|---|---|---|
| Task | `IT_kwDOCPuvuM4BPtZY` | A specific piece of work |
| Bug | `IT_kwDOCPuvuM4BPtZb` | An unexpected problem or behavior |
| Feature | `IT_kwDOCPuvuM4BPtZe` | A request, idea, or new functionality |
| Documentation | `IT_kwDOCPuvuM4BsnJj` | Explanations of how or why to do things |
| Infrastructure | `IT_kwDOCPuvuM4BsnKU` | Infrastructure of a project, like GitHub Actions |
